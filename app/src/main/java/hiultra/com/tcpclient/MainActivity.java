...
import hiultra.com.tcpclient.layout.ConsoleLayout;
...
//            String text = tcpclientEdit.getText().toString().trim();
            byte[] bytes = {0X02, 0X03, 0X04, 0X05, 0X00, 0X0B, 0X00, 0X58, 0X44,0X00, (byte) 0XB5};

            if (xTcpClient != null) {
                xTcpClient.sendMsg(tcpclientSwitchReconnect.isChecked()+"");
            } else {
                addMsg("还没有连接到服务器");
            }
        }
